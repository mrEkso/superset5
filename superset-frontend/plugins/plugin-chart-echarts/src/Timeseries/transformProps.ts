...
export default function transformProps(
  ...
  // Initialize legend state with default selected metrics
  // If defaultSelectedMetrics is set and legendState is empty, only show specified metrics
  let effectiveLegendState = legendState;
  const defaultMetricsArray = Array.isArray(defaultSelectedMetrics) ? defaultSelectedMetrics : [];
  if (
    (!legendState || Object.keys(legendState).length === 0) &&
    defaultMetricsArray.length > 0
  ) {
    effectiveLegendState = legendData.reduce(
      (acc, name) => ({
        ...acc,
        [String(name)]: defaultMetricsArray.includes(String(name)),
      }),
      {} as LegendState,
    );
  }
