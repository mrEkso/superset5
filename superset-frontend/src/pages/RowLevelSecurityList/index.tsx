...
function RowLevelSecurityList(props: RLSProps) {
  ...
  function handleRuleDuplicate(rule: RLSObject) {
    return SupersetClient.post({
      endpoint: `/api/v1/rowlevelsecurity/${rule.id}/duplicate`,
    }).then(
      ({ json }) => {
        refreshData();
        addSuccessToast(t('Duplicated: %s', json.message));
      },
      createErrorHandler(errMsg =>
        addDangerToast(t('There was an issue duplicating %s: %s', rule.name, errMsg)),
      ),
    );
  }
              ...
              {canWrite && (
                ...
                >
                  <span
                    ...
                    tabIndex={0}
                    className="action-button"
                    onClick={() => handleRuleDuplicate(original)}
                    data-test="rls-list-duplicate-icon"
                  >
                    <Icons.Copy />
                  </span>
                </Tooltip>
