...
class TopNUsersStrategy(TopNDashboardsStrategy):
    """
    Warm up charts in the top-n dashboards for specific users.
    
    Priority of user selection:
    1. `usernames` argument (explicit list)
    2. `role_name` argument (all users with this role)
    3. If neither provided: Users active in Logs since `since` time.
    """
    ...
    def __init__(
        ...
        super().__init__(top_n, since)
        self.usernames = usernames or []
        self.role_name = role_name

    def get_tasks(self) -> list[CacheWarmupTask]:
        # Get standard tasks (which use the owner)
        base_tasks = super().get_tasks()

        target_usernames = set(self.usernames)

        # 1. Role based
        if self.role_name:
            role = security_manager.find_role(self.role_name)
            if role:
                target_usernames.update([user.username for user in role.users])
            else:
                logger.warning(f"Role {self.role_name} not found")

        # 2. Activity based (fallback if no specific users/roles defined)
        if not target_usernames and self.since:
             User = security_manager.user_model
             active_users = (
                db.session.query(User.username)
                .join(Log, User.id == Log.user_id)
                .filter(Log.dttm >= self.since)
                .group_by(User.username)
                .all()
             )
             target_usernames.update([u[0] for u in active_users])

        # If still no users, return base tasks (standard behavior -> owner)
        if not target_usernames:
            return base_tasks

        # Expand each task for every target user
        new_tasks = []
        for task in base_tasks:
            for username in target_usernames:
                # Create a copy with the specific username
                new_tasks.append({"payload": task["payload"], "username": username})
        return new_tasks
