...
from superset.commands.security.duplicate import DuplicateRLSRuleCommand
...
class RLSRestApi(BaseSupersetModelRestApi):
    ...
    @expose("/<int:pk>/duplicate", methods=("POST",))
    @protect()
    @safe
    @statsd_metrics
    @event_logger.log_this_with_context(
    ...
    def duplicate(self, pk: int) -> Response:
        """Duplicate an RLS rule.
        ---
        post:
          summary: Duplicate an RLS rule
          description: >-
            Creates a deep copy of an existing RLS rule with a unique name
            and empty roles list.
          parameters:
          - in: path
            schema:
              type: integer
            name: pk
            description: The Rule pk to duplicate
          responses:
            201:
              description: RLS Rule duplicated
              content:
                application/json:
                  schema:
                    type: object
        ...
        """
        try:
            new_model = DuplicateRLSRuleCommand(pk).run()
            return self.response(
                201,
                id=new_model.id,
                message=f"Duplicated as '{new_model.name}'",
            )
        except RLSRuleNotFoundError:
            return self.response_404()
