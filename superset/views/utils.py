...
def get_native_filter_extra_form_data(
    slice_id: int, dashboard_id: int
) -> dict[str, Any]:
    """
    Get extra form data from Native Filters with default values.

    This function reads the native_filter_configuration from a dashboard's metadata,
    checks which filters apply to the given slice (chart), and collects their
    defaultDataMask.extraFormData to be applied during cache warmup.

    :param slice_id: The ID of the chart/slice
    :param dashboard_id: The ID of the dashboard
    :returns: A merged extraFormData dict containing time_range, filters, adhoc_filters
    """
    dashboard = db.session.query(Dashboard).filter_by(id=dashboard_id).one_or_none()
    ...
    # Get the chart to check its datasource
    chart = db.session.query(Slice).filter_by(id=slice_id).one_or_none()
    if not chart:
        return {}
    ...
    with contextlib.suppress(json.JSONDecodeError):
        json_metadata = json.loads(dashboard.json_metadata)
        native_filter_config = json_metadata.get("native_filter_configuration", [])

        for nf in native_filter_config:
            # Check if filter has default values
            default_data_mask = nf.get("defaultDataMask", {})
            ...
            if not targets:
                # No targets means filter applies to all charts (global filter)
                filter_applies = True
            else:
                for target in targets:
                    target_dataset_id = target.get("datasetId")
                    # If no specific dataset, filter applies globally
                    if target_dataset_id is None:
                        filter_applies = True
                        break
                    # Check if chart's datasource matches filter's target
                    if target_dataset_id == chart_datasource_id:
                        filter_applies = True
                        break

            if filter_applies:
                # Merge extraFormData
                # time_range: last one wins
                if "time_range" in extra_form_data:
                    merged_extra_form_data["time_range"] = extra_form_data["time_range"]

                # filters: accumulate
                if "filters" in extra_form_data:
                    if "filters" not in merged_extra_form_data:
                        merged_extra_form_data["filters"] = []
                    merged_extra_form_data["filters"].extend(extra_form_data["filters"])

                # adhoc_filters: accumulate
                if "adhoc_filters" in extra_form_data:
                    if "adhoc_filters" not in merged_extra_form_data:
                        merged_extra_form_data["adhoc_filters"] = []
                    merged_extra_form_data["adhoc_filters"].extend(
                        extra_form_data["adhoc_filters"]
                    )

                # Other keys: last one wins
                for key in extra_form_data:
                    if key not in ("time_range", "filters", "adhoc_filters"):
                        merged_extra_form_data[key] = extra_form_data[key]

    return merged_extra_form_data
